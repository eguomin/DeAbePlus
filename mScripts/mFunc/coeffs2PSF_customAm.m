function PSF = coeffs2PSF_customAm(p,coeffs, Sx, pixelSize, lambda, NA, Sz, zStepSize, RI)
% calculate PSF (2D or 3D) from Zernike coefficients (OSA/ANSI convention)
% and customized intensity distribution at pupil (here Gaussion for line
% confocal excitation as an example)
%
% Output
...
% customize the intensity distribution: Gaussian as an example
[X,~] = meshgrid(linspace(-1,1,Sx)/2,linspace(-1,1,Sy)/2);
w0=0.001;
mask_Gaussian = exp(-X.^2/w0^2);
pupilMask = pupilMask.*mask_Gaussian;

% final pupil function
